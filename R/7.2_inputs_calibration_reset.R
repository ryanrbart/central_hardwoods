# Inputs: Parameter and worldfile reset for fire calibration
...
misc_per_watershed_table <- watershed_table %>% 
  dplyr::mutate(
    ...
    def_file_parameters_out_pathname = file.path("out_r", watershed, "def_file_parameters_fire_-1"),
...
misc_per_site_table <- watershed_site_table %>% 
  dplyr::mutate(
    ...
    worldfile_name_new = file.path("ws", watershed, "worldfiles", paste0(watershed, "_", site, "_spinup_fire"), paste0(watershed, "_", site, "_spinup_fire.world"))
  )
